...
class UnitTesting
	...
	def method_sample_find_all_geos_in_region
		@sikuli.switch_app("Chrome")
		region = Region.new(490,280,500,400)      # x,y,w,h   pierwsze dwa to coordy
		# lewego gornego wierzcholka, pozostale to wymiary regionu; osie jak na normalnym
		# ukladzie wspolrzednych tylko oY idzie w dol - lewy gorny rog ekranu to (0,0)
	...
	def method_sample_star_menu_scroll_button
		@sikuli.switch_app("Chrome")
		region = Region.new(1050,720,90,90)      # x,y,w,h   pierwsze dwa to coordy
		# lewego gornego wierzcholka, pozostale to wymiary regionu; osie jak na normalnym
		# ukladzie wspolrzednych tylko oY idzie w dol - lewy gorny rog ekranu to (0,0)
	...
	def click_on_empty_fields#convienience method do testowania coordow klikalnych
		# obiektow
		@sikuli.switch_app("Chrome")
	...
	def test_coords_from_csv
		...
		CSV.foreach(@csv_path+"/ironmines2.csv") do |row|
			...
			@csv_hash[row[0].to_i] << [row[1].to_i, row[2].to_i] #tu powinno byc dopisywanie
			# a nie nadpisywanie
			@counts[row[0].to_i] +=1
		end
	...
	def display
		...
		#
		#@screen.type(Location.new(800,600), "6")    #  hack : w kazdym sektorze jest
		# ulepszony magazyn i jest dobrze widoczny, wiec mozna klikac na magazyn zeby
		# zebrac focus i potem zmienic sektor
	end
